/** @format */

import Rocky from '../assets/content-master/Rocky.png';
import Lyra from '../assets/content-master/Lyra.png';
import Death2x1 from '../assets/content-master/Death 2x1.png';
import Nyxarion from '../assets/content-master/Nyxarion.png';
import Sky from '../assets/content-master/Sky.png';
import AngryBirdConejo from '../assets/content-master/Angry Bird y Conejo.png';
import Veyra from '../assets/content-master/Veyra.png';
import Yuriko from '../assets/content-master/Yurico.png';

import nivel1 from '../assets/content-master/Nivel1.png';
import nivel2 from '../assets/content-master/Nivel2.png';
import nivel3 from '../assets/content-master/Nivel3.png';
import nivel4 from '../assets/content-master/Nivel4.png';
import nivel5 from '../assets/content-master/Nivel5_gris.png';
import nivel6 from '../assets/content-master/Nivel6.png';

const contentMasterYear = [
	{
		id: '1',
		title: 'KOALA',
		image: nivel1,
		description: 'Crea este fantástico Rig desde cero en Maya y llevémoslo juntos a Unreal.',
		tech: [],
		video: 'https://www.youtube.com/watch?v=KejwiSOTbHg&ab_channel=LIBELACADEMY',
		items: [
			{
				item: 'Tiempo de reto: 7 días.',
				more: null,
			},
			{
				item: 'Acceso 1 mes.',
				more: null,
			},
			{
				item: 'Acompañamiento en vivo.',
				more: null,
			},
			{
				item: 'Acceso a discord.',
				more: null,
			},
			{
				item: 'Participa por beca parcial o completa.',
				more: null,
			},
		],
		temario: 'https://drive.google.com/file/d/1l3u-wmuoIymUBzk_nyjwZmupGnRE-EFC/view?usp=sharing',
		level: 'Nivel 1',
		buttonName: 'Inicia el reto',
		button: true,
		options: [
			{
				id: 0,
				title: 'Ver Oferta',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: false,
				active: false,
			},
			{
				id: 1,
				title: '233,3 USD',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: true,
				active: false,
			},
			{
				id: 2,
				title: '5,99 USD',
				price: '14.99 USD',
				urls: {
					paypal: 'https://pay.hotmart.com/C103731235O?off=o4l9uatc',
					card: 'https://checkout.wompi.co/l/APM5Wz',
				},
				line: false,
				active: true,
				item: 'Acceso 1 año',
			},
		],
		modal: {
			modalCopy:
				'¡Estás a un paso de comenzar! Da el primer paso con toda la energía y compite por tu beca.',
			modalButton: '¡Empieza ahora!',
		},
	},
	{
		id: '2',
		title: 'Zoru',
		image: nivel2,
		description:
			'Vive una experiencia con la última versión de Blender y libera el gran talento que hay en ti.',
		tech: [],
		video: 'https://www.youtube.com/watch?v=Zvrgt3T4k90&ab_channel=LIBELACADEMY',
		items: [
			{
				item: 'Tiempo de reto: 7 días.',
				more: null,
			},
			{
				item: 'Acceso 1 mes.',
				more: null,
			},
			{
				item: 'Acompañamiento en vivo.',
				more: null,
			},
			{
				item: 'Acceso a discord.',
				more: null,
			},
			{
				item: 'Participa por beca parcial o completa.',
				more: null,
			},
		],
		temario: 'https://drive.google.com/file/d/1l3u-wmuoIymUBzk_nyjwZmupGnRE-EFC/view?usp=sharing',
		level: 'Nivel 2',
		buttonName: 'Sube el nivel 2',
		button: true,
		options: [
			{
				id: 0,
				title: 'Ver Oferta',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: false,
				active: false,
			},

			{
				id: 1,
				title: '199,9 USD',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: true,
				active: false,
			},
			{
				id: 2,
				title: '5,99 USD',
				price: '14.99 USD',
				urls: {
					paypal: 'https://pay.hotmart.com/N103731385R?off=t3ub5js9',
					card: 'https://checkout.wompi.co/l/Bvv56p',
				},
				line: false,
				active: true,
				item: 'Acceso 1 año',
			},
		],
		modal: {
			modalCopy:
				'¡No te detengas! Para desbloquear este reto, primero debes completar con éxito el Nivel 1.',
			modalButton: 'Acceder aquí',
		},
	},
	{
		id: '3',
		title: 'Cerdito',
		image: nivel3,
		description:
			'Aprende a crear personajes en Maya desde cero renderízalo en Arnold, aplica texturas en substance y más.',
		tech: [],
		video: 'https://www.youtube.com/watch?v=y4H2fP_p8jQ&ab_channel=LIBELACADEMY',
		items: [
			{
				item: 'Tiempo de reto: 7 días.',
				more: null,
			},
			{
				item: 'Acceso 1 mes.',
				more: null,
			},
			{
				item: 'Acompañamiento en vivo.',
				more: null,
			},
			{
				item: 'Acceso a discord.',
				more: null,
			},
			{
				item: 'Participa por beca parcial o completa.',
				more: null,
			},
		],
		temario: 'https://drive.google.com/file/d/1l3u-wmuoIymUBzk_nyjwZmupGnRE-EFC/view?usp=sharing',
		level: 'Nivel 3',
		buttonName: 'Sube el nivel 3',
		button: true,
		options: [
			{
				id: 0,
				title: 'Ver Oferta',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: false,
				active: false,
			},
			{
				id: 1,
				title: '166,6 USD',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: true,
				active: false,
			},
			{
				id: 2,
				title: '5,99 USD',
				price: '18.99 USD',
				urls: {
					paypal: 'https://pay.hotmart.com/Y95845229L?off=ckjcb9yf',
					card: 'https://checkout.wompi.co/l/TbmxvN',
				},
				line: false,
				active: true,
				item: 'Acceso 1 año',
			},
		],
		modal: {
			modalCopy: 'El camino continúa. Asegúrate de haber superado los Niveles 1 y 2.',
			modalButton: 'Acceder aquí',
		},
	},
	{
		id: '4',
		title: 'Angry Bird',
		image: nivel4,
		tech: [],
		video: 'https://www.youtube.com/watch?v=iLfsvgChDNY&ab_channel=LIBELACADEMY',
		items: [
			{
				item: 'Tiempo de reto: 10 días.',
				more: null,
			},
			{
				item: 'Acceso 1 mes.',
				more: null,
			},
			{
				item: 'Acompañamiento en vivo.',
				more: null,
			},
			{
				item: 'Acceso a discord.',
				more: null,
			},
			{
				item: 'Participa por beca parcial o completa.',
				more: null,
			},
		],
		temario: 'https://drive.google.com/file/d/1l3u-wmuoIymUBzk_nyjwZmupGnRE-EFC/view?usp=sharing',
		level: 'Nivel 4',
		buttonName: 'Sube el nivel 4',
		button: true,
		options: [
			{
				id: 0,
				title: 'Ver Oferta',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: false,
				active: false,
			},
			{
				id: 1,
				title: '133,3 USD',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: true,
				active: false,
			},
			{
				id: 2,
				title: '5,99 USD',
				price: '49 USD',
				urls: {
					paypal: 'https://pay.hotmart.com/C95840361M?off=ua213ddi',
					card: 'https://pay.hotmart.com/C95840361M?off=ua213ddi',
				},
				line: false,
				active: true,
				item: 'Acceso 1 año',
			},
		],
		modal: {
			modalCopy:
				'Estás cada vez más cerca. Este nivel requiere que hayas aprobado los Niveles 1, 2 y 3.',
			modalButton: 'Acceder aquí',
		},
	},
	{
		id: '5',
		title: 'Night Rabbit',
		image: nivel5,
		description: 'Crea este fantástico Rig desde cero en Maya y llevémoslo juntos a Unreal.',
		tech: [],
		video: 'https://www.youtube.com/watch?v=FRB0vkEUTZg&ab_channel=LIBELACADEMY',
		items: [
			{
				item: 'Tiempo de reto: 15 días.',
				more: null,
			},
			{
				item: 'Acceso 1 mes.',
				more: null,
			},
			{
				item: 'Acompañamiento en vivo.',
				more: null,
			},
			{
				item: 'Acceso a discord.',
				more: null,
			},
			{
				item: 'Participa por beca parcial o completa.',
				more: null,
			},
		],
		temario: 'https://drive.google.com/file/d/1l3u-wmuoIymUBzk_nyjwZmupGnRE-EFC/view?usp=sharing',
		level: 'Nivel 5',
		buttonName: 'Sube el nivel 5',
		button: false,
		options: [
			{
				id: 0,
				title: 'Ver Oferta',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: false,
				active: false,
			},
			{
				id: 1,
				title: '99,9 USD',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: true,
				active: false,
			},
			{
				id: 2,
				title: '5,99 USD',
				price: '69 USD',
				urls: {
					paypal: 'https://pay.hotmart.com/R95841256W?off=rlf1hup5',
					card: 'https://checkout.wompi.co/l/0DU6xV',
				},
				line: false,
				active: true,
				item: 'Acceso 1 año',
			},
		],
		modal: {
			modalCopy: '¡Nivel experto! Solo disponible para quienes ya conquistaron los Niveles 1 al 4.',
			modalButton: 'Acceder aquí',
		},
	},
	{
		id: '6',
		title: 'PACK LIBEL UP',
		image: nivel6,
		description:
			'Vive una experiencia con la última versión de Blender y libera el gran talento que hay en ti.',
		tech: [],
		video: 'https://www.youtube.com/watch?v=Zvrgt3T4k90&ab_channel=LIBELACADEMY',
		items: [
			{
				item: 'Acceso a los 5 niveles LIBEL UP.',
				more: null,
			},
			{
				item: 'Acceso 6 meses.',
				more: null,
			},
			{
				item: 'Acompañamiento en vivo.',
				more: null,
			},
			{
				item: 'Acceso a Discord.',
				more: null,
			},
		],
		temario: 'https://drive.google.com/file/d/1l3u-wmuoIymUBzk_nyjwZmupGnRE-EFC/view?usp=sharing',
		level: 'RETO',
		buttonName: '¡Inicia el reto!',
		button: true,
		options: [
			{
				id: 0,
				title: 'Ver Oferta',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: false,
				active: false,
			},
			{
				id: 1,
				title: '99,9 USD',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: true,
				active: false,
			},
			{
				id: 2,
				title: '29,95 USD',
				price: '',
				urls: {
					paypal: 'https://pay.hotmart.com/H104779741X?off=9ivfm9ty',
					card: 'https://checkout.wompi.co/l/pvj9Dw',
				},
				line: false,
				active: true,
				item: 'Acceso 1 año',
			},
		],
		modal: {
			modalCopy:
				'Accede al Pack completo de los 5 niveles Libel UP. Tendrás acceso 5 meses desde tu compra para completar el reto (revisa términos y condiciones en el temario).',
			modalButton: '¡Comienza ahora!',
		},
	},
];

export const contentMasterYear2 = [
	{
		id: '01',
		title: 'Veyra en ZBrush y CC5',
		image: Veyra,
		description: 'Crea este fantástico Rig desde cero en Maya y llevémoslo juntos a Unreal.',
		tech: [],
		video: 'https://www.youtube.com/watch?v=KejwiSOTbHg&ab_channel=LIBELACADEMY',
		items: [
			{
				item: 'Acceso 1 año',
				more: null,
			},
			{
				item: 'Aprende a tu ritmo.',
				more: 'Después de finalizar el 3D CAMP todos los accesos serán desactivados. Si deseas conservar el acceso al contenido del(de los) entrenamiento(s), debes comprar el plan año.',
			},
			{
				item: 'Clases en vivo',
				more: 'Podrás acceder a uno de los entrenamientos del campamento (el que elijas). Recuerda que si eliges Maya, tendrás un módulo adicional de Substance Painter.',
			},
			{
				item: 'Grabación de clases.',
				more: '',
			},
			// { item: 'Participa por BECAS', more: null },
		],
		temario: 'https://drive.google.com/file/d/1l3u-wmuoIymUBzk_nyjwZmupGnRE-EFC/view?usp=sharing',
		level: 'Transition',
		options: [
			{
				id: 0,
				title: 'Ver Oferta',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: false,
				active: false,
			},
			{
				id: 1,
				title: '233,3 USD',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: true,
				active: false,
			},
			{
				id: 2,
				title: '59,99 USD',
				price: '120 USD',
				urls: {
					paypal:
						'https://libel.academy/pago-paypal/payment.html?YW1vdW50PTU5Ljk5JmlkPTUwJSBEdG8gVmV5cmEgWkJydXNoIFRyYW5zaXRpb24gQWNjZXNvIEHxbw==',
					card: 'https://checkout.wompi.co/l/sPRjKj',
				},
				line: false,
				active: true,
				item: 'Acceso 1 año',
			},
		],
	},
	{
		id: '02',
		title: 'Yuriko en Blender, CC5 y Unreal Engine',
		image: Yuriko,
		description:
			'Vive una experiencia con la última versión de Blender y libera el gran talento que hay en ti.',
		tech: [],
		video: 'https://www.youtube.com/watch?v=Zvrgt3T4k90&ab_channel=LIBELACADEMY',
		items: [
			{
				item: 'Acceso 1 año',
				more: null,
			},
			{
				item: 'Aprende a tu ritmo.',
				more: 'Después de finalizar el 3D CAMP todos los accesos serán desactivados. Si deseas conservar el acceso al contenido del(de los) entrenamiento(s), debes comprar el plan año.',
			},
			// {
			// 	item: 'Feedback en vivo',
			// 	more: 'Podrás acceder a uno de los entrenamientos del campamento (el que elijas). Recuerda que si eliges Maya, tendrás un módulo adicional de Substance Painter.',
			// },
			{
				item: 'Grabación de clases.',
				more: '',
			},
			// { item: 'Participa por BECAS', more: null },
		],
		temario: 'https://drive.google.com/file/d/1l3u-wmuoIymUBzk_nyjwZmupGnRE-EFC/view?usp=sharing',
		level: 'Transition',
		options: [
			{
				id: 0,
				title: 'Ver Oferta',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: false,
				active: false,
			},
			{
				id: 1,
				title: '199,9 USD',
				price: null,
				urls: {
					paypal: null,
					card: null,
				},
				line: true,
				active: false,
			},
			{
				id: 2,
				title: '59,99 USD',
				price: '120 USD',
				urls: {
					paypal:
						'https://libel.academy/pago-paypal/payment.html?YW1vdW50PTU5Ljk5JmlkPTUwJSBEdG8gWXVyaWtvIEJsZW5kZXIgVHJhbnNpdGlvbiBBY2Nlc28gQfFv',
					card: 'https://checkout.wompi.co/l/fS04YO',
				},
				line: false,
				active: true,
				item: 'Acceso 1 año',
			},
		],
	},
];

export default contentMasterYear;
